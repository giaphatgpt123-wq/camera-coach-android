...
class CameraController(private val context:Context){
 private var selector=CameraSelector.DEFAULT_BACK_CAMERA;private var camera:Camera?=null;private var imageCapture:ImageCapture?=null;private var videoCapture:VideoCapture<Recorder>?=null;private var recording:Recording?=null
 ...
 fun bind(owner:LifecycleOwner,view:PreviewView,analyzer:ImageAnalysis.Analyzer?=null){
  ProcessCameraProvider.getInstance(context).addListener({
   ...
   val analysis=analyzer?.let{ImageAnalysis.Builder().setBackpressureStrategy(ImageAnalysis.STRATEGY_KEEP_ONLY_LATEST).setTargetResolution(android.util.Size(640,480)).build().also{a->a.setAnalyzer(ContextCompat.getMainExecutor(context),it)}}
